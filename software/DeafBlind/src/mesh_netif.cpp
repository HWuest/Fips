...
static esp_err_t mesh_netif_transmit_from_root_ap(void *h, void *buffer, size_t len)
{
    ...
    mesh_netif_driver_t mesh_driver = (mesh_netif_driver*) h;
...
{
    mesh_netif_driver_t driver = (mesh_netif_driver*) args;
...
{
    mesh_netif_driver_t driver = (mesh_netif_driver*) calloc(1, sizeof(struct mesh_netif_driver));
    if (driver == NULL) {
    ...
    if (!receive_task_is_running) {
        receive_task_is_running = true;
        xTaskCreate(receive_task, "netif rx task", 4096, NULL, 5, NULL);
    }
...
static esp_err_t start_wifi_link_sta(void)
{
    ...
    if ((ret = esp_wifi_register_if_rxcb((wifi_netif_driver*)driver,  esp_netif_receive, netif_sta)) != ESP_OK) {
...
esp_err_t mesh_netif_start_root_ap(bool is_root, uint32_t addr)
{
    if (is_root) {
        if (netif_ap) {
            ...
            mesh_delete_if_driver((mesh_netif_driver_t) esp_netif_get_io_driver(netif_ap));
        ...
        start_mesh_link_ap();
 //       ip_napt_enable(g_mesh_netif_subnet_ip.ip.addr, 1); // DHS FORWARD = 0 therefore ip_napt_enable function not defined
    }
    return ESP_OK;
}
...
esp_err_t mesh_netifs_start(bool is_root)
{
    if (is_root) {
        ...
        if (netif_sta && strcmp(esp_netif_get_desc(netif_sta), "sta") == 0) {
            ...
            mesh_delete_if_driver((mesh_netif_driver*) esp_netif_get_io_driver((esp_netif_t*)netif_sta));
        ...
        if (netif_ap) {
            esp_netif_action_disconnected(netif_ap, NULL, 0, NULL);
            mesh_delete_if_driver((mesh_netif_driver*) esp_netif_get_io_driver((esp_netif_t*) netif_ap));
            esp_netif_destroy(netif_ap);
            netif_ap = NULL;
        }
...
esp_err_t mesh_netifs_stop(void)
{
    ...
    if (netif_ap) {
        esp_netif_action_disconnected(netif_ap, NULL, 0, NULL);
        mesh_delete_if_driver((mesh_netif_driver_t) esp_netif_get_io_driver((esp_netif_t*) netif_ap));
        esp_netif_destroy(netif_ap);
        netif_ap = NULL;
    }
...
{
    mesh_netif_driver_t mesh =  (mesh_netif_driver*) esp_netif_get_io_driver((esp_netif_t*)netif_sta);
